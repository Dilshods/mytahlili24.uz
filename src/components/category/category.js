...
const Category = () => {
  ...
  const [vitalRes, setVitalRes] = useState([]);
  const [vitalPageNum, setVitalPageNum] = useState(1);
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    fetch(
      `http://165.232.85.45:1988/koinot/news/byCategoryId/${id}?page=0&size=10&sort=eventTime`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => setNewsCategory(result.object.content))
  ...
  // http://165.232.85.45:1988/koinot/news/maxSee?page=1&size=10&sort=see
  const handleinformations = () => {
    ...
    fetch(
      `http://165.232.85.45:1988/koinot/news/maxSee?page=${vitalPageNum}&size=10&sort=see`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        setVitalPageNum(vitalPageNum + 1);
        setVital([...vital, ...result.object.content]);
        setVitalRes(result.object.content.length !== 0);
      })
      .catch((error) => console.log("error", error));
  };
  // console.log("vitalPageNum", vital, vitalPageNum, vitalRes);
  return (
    <>
      <div className="container">
        <div className="nav_map">
          ...
          {newsCategory.slice(0, 1).map((cate) => (
            <span className="this_page">{cate.category.textUz}</span>
          ))}
        </div>
      </div>
      <main>
        <div className="container_news">
          {newsCategory.length > 0 ? (
            <section className="mainContentPage">
              {newsCategory.slice(0, 1).map((cat) => {
                return (
                  <div className="top_big-carded">
                    <div
                      id="big_card-news"
                      className="big_card_img_contaner"
                      style={{
                        background: `url(${cat?.attachment?.link} )`,
                        border: "1px solid",
                      }}
                    >
                      <a href={`/news/${cat.id}`}>
                        <div className="big__card">
                          <div className="post_inners">
              ...
              <div className="grid-container">
                {newsCategory.slice(1).map((cat) => (
                  <div className="card_normal_wrap " key={cat.id}>
